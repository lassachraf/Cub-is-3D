...
/*   Updated: 2024/10/13 19:04:19 by alassiqu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    cub->player->tex = ft_calloc(1, sizeof(t_tex)); // Allocate player texture once
...
void    put_player(t_cub3d *cub)
{
    ...
    if (cub->player->tex == NULL || cub->player->tex->img == NULL)
        return; // Return if the player image hasn't been loaded

    i = 0;
    while (i < cub->player->tex->height && i < cub->hov) // Ensure we don't exceed the image boundary
    {
        j = 0;
        while (j < cub->player->tex->width && cub->player->x + j < cub->wov) // Ensure we don't exceed the image boundary
        {
            color = get_texture_color(cub->player->tex, i, j); // Get color from player texture
            printf("color > %x\n", color);
            if (color != 0xFF000000) // Skip fully transparent pixels (if your image has transparency)
