...
/*   Updated: 2024/11/05 16:22:21 by alassiqu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void draw_wall_slice(t_cub3d *cub, t_ray ray, int x)
{
    ...
    // Set tex_x based on wall direction and wall_x value
    wall.tex_x = wall.wall_x * (double)texture->width;
    
    // Adjust tex_x based on the wall's orientation and ray direction
    if (cub->side == 0 && ray.ray_y < 0)  // North wall adjustment
        wall.tex_x = texture->width - wall.tex_x - 1;
    else if (cub->side == 1 && ray.ray_y > 0)  // South wall adjustment
        wall.tex_x = texture->width - wall.tex_x - 1;
    else if (cub->side == 2 && ray.ray_x < 0)  // West wall adjustment
        wall.tex_x = texture->width - wall.tex_x - 1;
    else if (cub->side == 3 && ray.ray_x > 0)  // East wall adjustment
        wall.tex_x = texture->width - wall.tex_x - 1;
